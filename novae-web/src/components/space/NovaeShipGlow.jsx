...
export function NovaeShipGlow({ position, isThrusting, isBoosting, isBraking, index, isMain = false }) {
  ...
  useFrame(({ clock }) => {
    ...
    const power = braking ? 0 : boosting ? 1.3 : thrusting ? 1 : 0.6
    ...
    coreRef.current.material.opacity = THREE.MathUtils.lerp(coreRef.current.material.opacity, braking ? 0 : thrusting ? 0.95 : 0.4, 0.15)
    haloRef.current.material.opacity = THREE.MathUtils.lerp(haloRef.current.material.opacity, braking ? 0 : thrusting ? 0.6 : 0.24, 0.15)
    coreRef.current.material.color.lerp(coreColorTarget.set(thrusting ? '#ffffff' : '#3355aa'), 0.12)
    haloRef.current.material.color.lerp(haloColorTarget.set(boosting ? '#aa55ff' : thrusting ? '#ff6600' : '#2244aa'), 0.12)
  })
