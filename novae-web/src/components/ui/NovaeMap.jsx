...
import { NOVAE_GATES, SOLAR_SYSTEM } from '../../utils/solarSystem'

const SIZE = 140

export function NovaeMap({ position, rotation, planets, dimension = 'solar' }) {
  ...
  useEffect(() => {
    ...
    if (dimension === 'solar') {
      ...
      planets.forEach((planet) => {
        ...
        context.beginPath()
        context.arc(x, y, Math.min(3, Math.max(1, planet.novaeWorldProps.size)), 0, Math.PI * 2)
        context.fill()
      })
    }

    NOVAE_GATES.filter((novaeGate) => novaeGate.sourceDimension === dimension).forEach((novaeGate) => {
      const [x, y] = toMap(novaeGate.position[0], novaeGate.position[2])
      context.strokeStyle = novaeGate.color
      context.beginPath()
    ...
    context.fill()
    context.restore()
  }, [dimension, planets, position, rotation])
